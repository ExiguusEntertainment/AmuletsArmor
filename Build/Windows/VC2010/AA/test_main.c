...
static unsigned char G_bytes24[TEXTURE_HEIGHT][TEXTURE_WIDTH][3];
static void *create_texture_24bit(void)
{
    unsigned char *p_bytes = G_bytes24[0][0];
    unsigned char *p = p_bytes;
    int x, y;

    for (y=0; y<TEXTURE_HEIGHT; y++) {
        for (x=0; x<TEXTURE_WIDTH; x++, p+=3) {
            if ((x==0) || (x==(TEXTURE_WIDTH-1)) || (y==0) || (y==(TEXTURE_HEIGHT-1))) {
                //edge of square
                // red
                p[0] = 255;
    ...
    G_bytes24[0][1][1] = 0;
    G_bytes24[0][1][2] = 0;
    G_bytes24[0][TEXTURE_WIDTH-1][1] = 0;
    G_bytes24[0][TEXTURE_WIDTH-1][2] = 0;
...
int test_main( int argc, char* argv[] )
{
     ...
     */
    setup_opengl( /*width, height*/ 640, 480 );
